...
internals.isValidRoute = function (request) {
    ...
    if (!Util.isUndefined(options.enabled)) {
...
internals.getPagination = function (request, config) {
    ...
    if (!Util.isUndefined(routeDefaults.pagination)) {
...
internals.onPreResponse = function (request, reply) {
    ...
    if (internals.isValidRoute(request) &&
        ...
        const totalCount = Util.isUndefined(source.totalCount) ?
                           request[internals.config.meta.totalCount.name] :
                           source.totalCount;

        let pageCount = null;
        if (!Util.isNil(totalCount)) {
        ...
        internals.assignIfActive(meta, 'totalCount', Util.isNil(totalCount) ? null : totalCount);

        const hasNext = !Util.isNil(totalCount) && totalCount !== 0 && currentPage < pageCount;
