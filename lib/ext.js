...
internals.onPreResponse = function (request, reply) {
    ...
    if (internals.isValidRoute(request) &&
        ...
        const results = Array.isArray(source) ? source : source[internals.config.reply.results.name];
        ...
        const totalCount = Utils.isUndefined(source[internals.config.reply.totalCount.name]) ?
                           request[internals.config.meta.totalCount.name] :
                           source[internals.config.reply.totalCount.name];
