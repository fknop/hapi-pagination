...
internals.onPreResponse = function (request, reply) {
    ...
    if (internals.isValidRoute(request) &&
        ...
        const results = Array.isArray(source) ? source : source[internals.config.results.name];
        ...
        const totalCount = Utils.isUndefined(source[internals.config.meta.totalCount.name]) ?
                           request[internals.config.meta.totalCount.name] :
                           source[internals.config.meta.totalCount.name];
